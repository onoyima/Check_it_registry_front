  ...
  const loadData = async () => {
    try {
      ...
      const [reportedRes, alertsRes] = await Promise.all([
        ...
        fetch('http://localhost:3006/api/admin-portal/alerts/device-checks?limit=50', { headers })
      ])
    ...
    <Layout requireAuth>
      <div className="container-fluid">
        <div className="d-flex align-items-center gap-2 mb-4">
          <ShieldAlert />
          <div>
            <h1 className="h4 m-0">Admin Alerts</h1>
                ...
                <tbody>
                  {reportedDevices.map(dev => (
                    <tr key={dev.id}>
                      ...
                      <td>
                        <a className="btn btn-sm btn-outline-primary" href={`/admin/devices/${dev.id}`}>
                  ...
                  {reportedDevices.length === 0 && (
                    <tr><td colSpan={7} className="text-center py-4 text-muted">No active reported devices</td></tr>
                  )}
                </tbody>
              </table>
            </div>
